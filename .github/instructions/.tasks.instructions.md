...
- Each module must ship with a `module.manifest.json` file in its module directory that describes how the module should be packaged. Keep the manifest tooling-agnostic JSON with the following fields:
  - `name`: lowercase module name matching the directory.
  - `version`: semantic version string for the module artifact.
  - `description`: short human-readable summary of the module purpose.
  - `include`: array of repo-relative file paths that must be present in the packaged artifact (include the manifest itself and any module-specific instruction files).
  - `exclude`: optional array of repo-relative paths to omit after inclusion.
  - `source`: optional object describing where the module originated. When present, include a `git` property with the canonical repository URL and, if tag naming deviates from `v<Version>`, a `tagPrefix` property that callers can prepend when constructing version tags.
  - Ensure paths are repository-relative so they remain valid when the archive is unpacked at the repo root.
