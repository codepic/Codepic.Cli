...
- Do not declare standalone PowerShell functions in task files; all executable logic should live inside Invoke-Build `task {}` scriptblocks or supporting begin/process/end scaffolding. When you need reusable helpers for a given task file, place them in the companion `*.tasks.functions.ps1` so contributors get comment-based help and IntelliSense while keeping the Invoke-Build surface readable.
- Surface guard failures with `Write-Error` instead of `throw` so the bootstrapper's `$ErrorActionPreference = 'Stop'` terminates execution while still emitting structured error records.
- Check for external command availability inline using the double-bang pattern and the `-ErrorAction Ignore` exception. The canonical approach is:

      ```
      # Test if az command exists
      !!(Get-Command -Name az -ErrorAction Ignore)
      # Test if az command does not exist
      !(Get-Command -Name az -ErrorAction Ignore)
      ```

  Apply this inline at the call site instead of wrapping it in helper functions, omit quotes around the command name so alias resolution works, and treat it as the sole sanctioned use of `-ErrorAction` within task files.
...
- If a task file exposes an alias parameter, ensure it defaults to a sensible value (currently `cc`) and check for existing aliases via `Get-Alias` before creating new ones. Use `{CLI}` in comment-based help and docs to remind operators to substitute their preferred alias.
- When adding aliases to user profiles, append a relative `Set-Alias` command (e.g., `Set-Alias {CLI} './.build.ps1'`) and reload the profile so the alias is available immediately. Handle conflicts gracefully.
